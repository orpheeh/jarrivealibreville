    ...
    async uploadFacture(ctx, _next) {
        ...
        try {
            if (type == 'achat_import') {
                const r = await UpdateCommande(id, { achat_import: { facture: file.path + ";" + file.name  + ";" + file.type} },
                    { commandeRepository });
            } else {
                const r = await UpdateCommande(id, { import: { facture: file.path + ";" + file.name + ";" + file.type} },
                    { commandeRepository });
            }
            ctx.status = 200;
        } catch (err) {
