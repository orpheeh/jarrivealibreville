...
module.exports = {
    ...
    async uploadFacture(ctx, _next) {
        ...
        try {
            if (type == 'achat_import') {
                const r = await UpdateCommande(id, { achat_import: { facture: file.path } },
                    { commandeRepository });
                console.log(r);
            } else {
                const r = await UpdateCommande(id, { import: { facture: file.path } },
                    { commandeRepository });
                console.log(r);
            }
            ctx.status = 200;
    ...
    async renderFacture(ctx, _next) {
        ...
        const path = commande.type == 'achat_import' ? commande.achat_import.facture :
