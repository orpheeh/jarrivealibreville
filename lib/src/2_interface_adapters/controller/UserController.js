...
module.exports = {
    ...
    async createUser(ctx, _next) {
        ...
        try {
            ...
            if (user) {
                ...
                await _ctx.render('inscription/index', { title: "Inscription - J'arrive à Libreville", error: "Impossible de créer l'utilisateur" })
            }
        } catch (err) {
            console.log(err);
            await ctx.render('inscription/index', { title: "Inscription - J'arrive à Libreville", error: "Cet email a déjà été utilisé" })
        }
    },
    ...
    async updateUser(ctx, _next) {
        ...
        try {
            ...
            console.log(err);
            ctx.status = 500;
            ctx.body = "Vous n'êtes pas authentifié sur ce navigateur !";
        }
    },
    ...
    async changePassworSendEmailAPI(ctx, _next) {
        ...
        if (!token) {
            ctx.status = 500;
            ctx.body = "Vous devez changer le mot de passe sur le navigateur sur lequel vous avez fait la demande";
            return;
        }
        try {
