module.exports = {
    ...
    async mon_compte(ctx, _next) {
        if (ctx.state.user) {
            ...
            await ctx.render('profil/index', { title: "Mon compte | J'arrive à Libreville", user });
        } else {
            await ctx.redirect("/");
        }
    },
    ...
    async achat_and_import(ctx, _next) {
        if (ctx.state.user) {
            ...
            await ctx.render('achat_import/index', { title: "Achat et Import | J'arrive à Libreville", user })
        } else {
    ...
    async import(ctx, _next) {
        if (ctx.state.user) {
            const user = ctx.state.user;
            await ctx.render('import/index', { title: "Import | J'arrive à Libreville", user })
        } else {
            ctx.redirect("/connexion");
        }
    },
